...
__global__ void predict_tiles_kernel(
  ...
  if (tile_class == 4) {
    ...
    if (threadIdx.x == 0) {
      ...
      tile_classes[tile] = activity_sum >= 8ull * edges ? 3 : 0;
    }
    __syncthreads();
...
torch::Tensor fastvid_encode_cuda(
  ...
  const int32_t scale = int32_t{1} << (bit_depth - 8);
  const int32_t fine_step = 1 + ((100 - quality) * scale + 11) / 12;
  const int32_t coarse_step = 1 + ((100 - quality) * scale + 4) / 5;
  uint8_t initial_class = 0;
  if (layout == 1 && bit_depth == 8) initial_class = 1;
  else if (layout == 0 && bit_depth >= 10) initial_class = 2;
  else if (layout == 2 && bit_depth == 16) initial_class = 2;
  else if (layout == 2 && bit_depth == 10) initial_class = 4;
  ...
  predict_tiles_kernel<<<tiles.size(), 256, 0, stream>>>(
      ...
      base_step, fine_step, coarse_step, tile_classes.data_ptr<uint8_t>(),
