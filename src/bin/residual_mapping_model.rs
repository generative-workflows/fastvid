use fastvid::{
    CodecOptions, Frame, Frame16, FrameRate, TileResidualMappingModel, analyze_residual_mapping,
    analyze_residual_mapping16, decode, decode_with_reference, decode16, decode16_with_reference,
    encode, encode_with_reference, encode16, encode16_with_reference,
};
use std::env;
use std::fs;
...
fn main() -> ExitCode {
    match run() {
        Ok(()) => ExitCode::SUCCESS,
        Err(error) => {
            eprintln!("residual_mapping_model: {error}");
            ExitCode::FAILURE
        }
    }
}
...
fn run() -> Result<(), Box<dyn std::error::Error>> {
    ...
    match arguments.first().map(String::as_str) {
        ...
        _ => Err(
            "usage: residual_mapping_model yuv422 SAMPLE INPUT WIDTH HEIGHT FPS FRAMES QUALITY THREADS GOP\n\
             or: residual_mapping_model yuv422p16le SAMPLE INPUT WIDTH HEIGHT FPS FRAMES BIT_DEPTH QUALITY THREADS GOP"
                .into(),
        ),
    }
}
...
fn model_8bit(arguments: &[String]) -> Result<(), Box<dyn std::error::Error>> {
    ...
    for (frame_index, bytes) in raw.chunks_exact(frame_len).enumerate() {
        ...
        print_models(
            ...
            quality,
            gop,
            &analyze_residual_mapping(&frame, reference, options)?,
        );
...
fn model_high_bit(arguments: &[String]) -> Result<(), Box<dyn std::error::Error>> {
    ...
    for (frame_index, bytes) in raw.chunks_exact(frame_len).enumerate() {
        ...
        print_models(
            ...
            quality,
            gop,
            &analyze_residual_mapping16(&frame, reference, options)?,
        );
        let encoded = if let Some(reference) = reference {
...
fn print_models(
    ...
    quality: u8,
    gop: usize,
    models: &[TileResidualMappingModel],
) {
    for (tile, model) in models.iter().enumerate() {
        println!(
            ...
            model.bounded_payload_bytes,
            model.actual_payload_bytes.min(model.bounded_payload_bytes),
        );
    }
}
