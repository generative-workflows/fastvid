#!/usr/bin/env bash
# Complete-byte 4-lane Rice model over the native high-bit supplement.
set -euo pipefail
...
temporary="$(mktemp /tmp/fastvid-rice-lane-model.XXXXXX)"
trap 'rm -f -- "$temporary"' EXIT

mkdir -p "$(dirname -- "$output")"
cargo build --release --bin entropy_model > /dev/null
first=1
