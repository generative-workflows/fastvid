#!/usr/bin/env bash
# EXP-0110 native high-bit version-2/version-5 comparison.
set -euo pipefail

script_dir="$(cd -- "$(dirname -- "${BASH_SOURCE[0]}")" && pwd)"
repo_dir="$(cd -- "$script_dir/.." && pwd)"
output="${1:-$repo_dir/artifacts/exp0110-full-tile-shards-ab.tsv}"
...
temporary="$(mktemp /tmp/fastvid-full-tile-shards-ab.XXXXXX)"
  ...
  while IFS=$'\t' read -r id path frames frame_rate width height bit_depth; do
    for quality in 90 100; do
      for variant in baseline bounded-full-tile; do
        ...
        if [[ "$variant" == bounded-full-tile ]]; then
          command=benchmark-yuv422p16le-parallel-full-tile
        fi
...
echo "results: $output"
python3 "$script_dir/summarize-full-tile-shards-ab.py" "$output"
