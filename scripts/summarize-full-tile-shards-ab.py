#!/usr/bin/env python3
"""Summarize EXP-0110 high-bit full-tile bounded-shard prototype."""
...
def main() -> None:
    ...
    for (sample, depth, quality), group_rows in grouped.items():
        for variant in ("baseline", "bounded-full-tile"):
    ...
    for sample, depth, quality in sorted(grouped):
        ...
        candidate = selected[(sample, depth, quality, "bounded-full-tile")]
        q90_results.append(
            (
    ...
    q100_exact = all(
        ...
        if key[2] == "100" and key[3] == "bounded-full-tile"
    )
    passed = aggregate_delta <= 0.01 and worst_sample_delta <= 0.02 and q100_exact
