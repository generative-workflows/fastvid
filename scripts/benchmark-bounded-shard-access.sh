#!/usr/bin/env bash
# EXP-0108 warm-cache independent tile access comparison.
set -euo pipefail

script_dir="$(cd -- "$(dirname -- "${BASH_SOURCE[0]}")" && pwd)"
repo_dir="$(cd -- "$script_dir/.." && pwd)"
output="${1:-$repo_dir/artifacts/exp0108-bounded-shard-access.tsv}"
...
temporary="$(mktemp /tmp/fastvid-bounded-shard-access.XXXXXX)"
...
echo "results: $output"
python3 "$script_dir/summarize-bounded-shard-access.py" "$output"
